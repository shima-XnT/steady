...
```text
PWA / Android WebView                      Google Apps Script
  ↓ 入力                                     ↓ doPost / doGet
...
Android 側の `app/src/main/assets` は root から同期します。root と assets の二重編集はしません。
...
- root の `index.html`, `manifest.json`, `sw.js` を assets へコピー
- root の `css/`, `js/`, `icons/` を assets へ再帰コピー
- 旧 `assets/www` を削除
- ハッシュ比較で root / assets の一致を検証
...
2. ウェブアプリとして再デプロイ
3. Android / PWA の設定画面で `gasSyncUrl` を更新

## 実機確認時の注意点

- Android 実機では、root 修正後に必ず `.\sync-assets.ps1` を実行してからビルドする
...
- `.\sync-assets.ps1` 実行後、root と `android/app/src/main/assets` の差分が 0 件であることを確認する
